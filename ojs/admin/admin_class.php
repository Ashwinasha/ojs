...
class Action
{
	...
	function custom_req()
	{
		...
		$check = $this->db->query("SELECT * FROM customization where product_id = $product_id and colour_id = $colour_id  and status NOT IN ('denied','accepted') and  size_id = $size_id  and user_id ={$_SESSION['login_id']} ");
		$id = $check->num_rows > 0 ? $check->fetch_array()['id'] : '';
		if (!empty($id))
			$save = $this->db->query("UPDATE customization set qty = qty+$qty where id = $id");
		else
			$save = $this->db->query("INSERT INTO customization set $data");
		if ($save) {
			return 1;
		}
	}
	function make_req()
	{
		...
		if (empty($order_item_id) || empty($product_id) || empty($request_type) || empty($message)) {
		...
		$request_type = mysqli_real_escape_string($this->db, $request_type);
		...
		if (isset($_FILES['images']) && count($_FILES['images']['name']) > 0) {
			...
			foreach ($_FILES['images']['tmp_name'] as $key => $tmp_name) {
				...
				$target_dir = "../assets/uploads/requests/";
				...
				if (move_uploaded_file($file_tmp, $target_file)) {
					...
					return "Error uploading file: $file_name";
				}
			}

			if (!empty($image_paths)) {
				$data .= ", images = '" . implode(',', $image_paths) . "' ";
			} else {
				$data .= ", images = NULL ";
			}
		} else {
		...
		$check = $this->db->query("SELECT * FROM service_requests WHERE order_item_id = $order_item_id AND request_type = '$request_type' AND status NOT IN ('denied', 'approved') AND user_id = $user_id");

		if ($check->num_rows > 0) {
			$request_id = $check->fetch_array()['id'];
			$save = $this->db->query("UPDATE service_requests SET $data WHERE id = $request_id");
		} else {
			$save = $this->db->query("INSERT INTO service_requests SET $data");
		}
	...
	function update_custom_req()
	{
		...
		$check = $this->db->query("SELECT * FROM customization WHERE id = $id");

		if ($check->num_rows > 0) {
			$row = $check->fetch_assoc();
			$id = $row['id'];

			$save = $this->db->query("UPDATE customization SET $data WHERE id = $id");
			...
			if ($status == 'accepted') {
				...
				if ($save) {
					$this->reduce_stock($row['product_id'], $row['qty']);
					...
					$order_item = $this->db->query("INSERT INTO order_items SET $data");
	...
	function update_user_req()
	{
		...
		$check = $this->db->query("SELECT * FROM service_requests WHERE id = $id");

		if ($check->num_rows > 0) {
			$row = $check->fetch_assoc();
			$id = $row['id'];

			$save = $this->db->query("UPDATE service_requests SET $data WHERE id = $id");
		} else {
			return 0;
		}
	...
	function upload_file()
{
    ...
    if (isset($move) && $move) {
        echo json_encode(array("status" => 1, "fname" => $fname));
    } else {
	...
	function save_product()
	{
		...
		if ($save) {
			...
			foreach ($fname as $k => $v) {
				$sourcePath = '../assets/uploads/products/' . $v;
				$destinationDir = '../assets/uploads/products/' . $item_code;
				...
				if (is_file($sourcePath)) {
					rename($sourcePath, $destinationDir . '/' . $v);
				}
			}
			return $id;
		}
	...
	{
		$query = $this->db->query("SELECT available_in_store FROM products WHERE id = '$product_id'");

		if ($query->num_rows > 0) {
			$row = $query->fetch_assoc();
			$current_stock = $row['available_in_store'];

			if ($current_stock >= $quantity) {
				$new_stock = $current_stock - $quantity;

				$update = $this->db->query("UPDATE products SET available_in_store = '$new_stock' WHERE id = '$product_id'");
